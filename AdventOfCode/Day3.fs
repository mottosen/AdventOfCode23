    ...
    static member Star1 (input : string[]) : string =
        let nextToSymbol (num : Match) (line : string) (neighbors : (string option)*(string option)) : bool =
            ...
            || match neighbors with
                | None, None -> false
                | Some l, None
                | None, Some l -> Regex.IsMatch(l[s1..s2], symbols)
        ...
        let lineSum (idx : int) (lines : string[]) : int =
            ...
            let neighbors = 
                (
                    (try Some (lines[idx-1]) with | _ -> None),
                    (try Some (lines[idx+1]) with | _ -> None)
                )

            (0, nums)
            ||> Seq.fold (fun acc num -> if (nextToSymbol num lines[idx] neighbors) then acc+int(num.Value) else acc)

        input
        |> Array.mapi (fun i line -> lineSum i input)
        |> Array.fold (fun acc sum -> acc + sum) 0
        |> string
    ...
    static member Star2 (input : string[]) : string =
        ...
        let gearRatio (gear : Match) (line : string) ((n1,n2) : (string option)*(string option)) : float =
            ...
            if (Seq.length neighbors) <> 2 then 0.0
                ...
                neighbors |> Seq.fold (fun acc f -> acc * f) 1.0
        ...
        let lineSum (idx : int) (lines : string[]) : double =
            ...
            (0.0, gears)
            ||> Seq.fold (fun acc gear -> acc + (gearRatio gear lines[idx] neighbors))

        input
        |> Array.mapi (fun i line -> lineSum i input)
        |> Array.fold (fun acc sum -> acc + sum) 0.0
        |> string